...
tmpm383fw_exe_path_r = r"{}".format(config_json_data["tmpm383fw_exe_path_dict"])
display_binary_path = r"{}".format(config_json_data["vs3t_display_bin_path_dict"])


print("tmpm380fd_exe_path_r=",tmpm383fw_exe_path_r)
...
def handle_initial_dialog(app_path):
    ...
    # Call the function
handle_initial_dialog(tmpm383fw_exe_path_r)
...
# click on setup menu and select communication
try:
    main_window_tool.menu_select("Setup-> Communication...")
    print("communication setup selected ")
...
try:
    click_on_setup_ok_button = main_window_tool.child_window(title="OK", auto_id="1", control_type="Button")
    click_on_setup_ok_button.click_input()
...
app = Application(backend='uia').start(tmpm383fw_exe_path_r)
main_window_tool = app.window()
...
try:
    ok2_button = main_window_tool.child_window(title="OK", control_type="Button")
    ok2_button.click()
    print("Successfully clicked 'OK' button.")
    # Return the app object for further automation
...
try:
    click_on_ok_button_after_flash = main_window_tool.child_window(title="No", auto_id="2", control_type="Button")
    click_on_ok_button_after_flash.click()
